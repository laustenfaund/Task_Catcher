# Note Goat assistant proxy

A Cloudflare Worker that stands between the [`hosted/`](../hosted) build of
Note Goat and Anthropic's API. It holds your real Anthropic API key as a
...
**Scope: this only covers the Anthropic connection** (Quick Capture's AI
cleanup and AI-assisted module setup). Note Goat's separate Google
connection (Sheets/Docs/Calendar sync) is untouched in `hosted/` — that
still needs each person's own Google Cloud project and OAuth credentials,
because sync writes to *that person's own* Google account, not a shared
one. See the root README for why that one isn't converted the same way.
...
## Photo attachments cost more than the estimate assumes

Quick Capture's "Clean up with AI" can attach a photo. This Worker's
pre-call cost estimate sizes a request by its raw JSON length, which is a
decent stand-in for plain text but overshoots for images — a real photo's
Anthropic token cost is based on pixel dimensions, typically far below
what its base64 size would suggest under a flat estimate. In practice this
means a photo-cleanup request might occasionally get refused as "over cap"
when the real call would have fit comfortably — safe (never lets a request
through it shouldn't), just occasionally overcautious. See the comment
above `estimateInputTokens` in `src/index.js` if you want to refine it.
...
enough. This is the same dashboard-only path used to set up the other
apps' Workers — no Node, no npm, no CLI required.

1. **Create the Worker.**
   - Dashboard → **Workers & Pages** → **Create** → **Workers** → give it
     a name (e.g. `note-goat-assistant` — this becomes part of its URL)
...
2. **Create two KV namespaces.**
   - Dashboard → **Storage & Databases** → **KV** → **Create a
     ...
     `note-goat-usage` and `note-goat-passcodes`. This namespace name is
     just a label in your account; it doesn't need to match anything in
     the code.
...
7. **Check the pricing table.** `PRICING` at the top of `src/index.js`
   has already been verified against Anthropic's published rate for
   `claude-sonnet-5`. If you add another model, or Anthropic changes its
   pricing, update the numbers there — the spending caps are only as
   accurate as this table.

8. **Copy the Worker's URL.** It's shown at the top of the Worker's
   overview page: `https://note-goat-assistant.<your-subdomain>.workers.dev`.
   Paste that into `ASSISTANT_PROXY_URL` near the bottom of the
   `<script>` block in `hosted/index.html`, just above the Claude API
   section.
