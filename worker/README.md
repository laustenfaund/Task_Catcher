...
5. **Verify the pricing table.** `PRICING` at the top of `src/index.js` is
   a placeholder — check your actual per-million-token rate for
   claude-sonnet-5 at `console.anthropic.com` (Settings → Billing) or the
   Anthropic pricing docs, and update it. The caps above are only as
...
6. **Deploy:**
   ...
   `https://note-goat-assistant.<your-subdomain>.workers.dev`. Paste that
   into `ASSISTANT_PROXY_URL` near the bottom of the `<script>` block in
   `hosted/index.html`, just above the Claude API section.
