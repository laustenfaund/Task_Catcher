// Note Goat hosted-assistant proxy
//
...
//
// NOTE ON PHOTO ATTACHMENTS: Quick Capture's "Clean up with AI" can attach
// a base64-encoded photo. This Worker's cost estimate (see
// estimateInputTokens below) sizes the request by its JSON length, which
// is a reasonable stand-in for plain text but overshoots badly for images
// — Anthropic actually bills an image by its pixel dimensions (roughly
// width*height/750 tokens), which is typically far fewer tokens than a
// photo's base64 size would suggest under a flat chars-per-token guess.
// That means photo-cleanup requests may get their pre-call budget
// reservation estimated much higher than they'll actually cost, and could
// occasionally be refused as "over cap" when the real call would have
// fit comfortably. This errs safe (never lets a request through it
// shouldn't) rather than open, so it's left as-is; if it becomes annoying
// in practice, raise PER_USER_DAILY_CAP_CENTS, or improve
// estimateInputTokens to special-case image content blocks.
//
...
// input price. Also verify this — Note Goat doesn't use caching today, so
// in practice this multiplier won't come into play, but it's here in case
// that changes.
const CACHE_WRITE_MULTIPLIER = 1.25;
const CACHE_READ_MULTIPLIER = 0.1;

// Note Goat asks for 700-800 max_tokens today — honor what the client
// asks for, but never above this, since a tampered client request could
// otherwise ask for far more than any real feature needs.
...
export default {
  async fetch(request, env) {
    ...
    // messages can contain image content blocks (Quick Capture's photo
    // attach) — forwarded through unchanged, same as text.
    const outgoing = {
      model,
...
// reservation errs toward over-counting rather than under-counting. See
// the top-of-file note: this overshoots further still for image content
// blocks, which is a known, accepted tradeoff.
